package core.baseComponent
{
	...
	public class IRButton extends Sprite implements SelectItem
	{
		 ...
		 * 
		 */		
		public function IRButton(arg:Array, centerZoom:Boolean = true, _isSelect:Boolean = true,_isStateChange:Boolean = false)
		{
			super();
			urlArr = arg;
		...
		private function loadOkHandler(event:Event):void
		{
			...
			try
			{
				...
				bitmapData =new BitmapData(loader._loaderContent[0].width,loader._loaderContent[0].height,true,0x000000);   
				bitmapData.draw(loader._loaderContent[0]); 
//				
//				for(var x:uint=0;x<loader._loaderContent[0].width;x++){  
//					for(var y:uint=0;y<loader._loaderContent[0].height;y++){  
//						if(bitmapData.getPixel32(x,y)){  
//							var alphaValue:uint=bitmapData.getPixel32(x,y)>> 24 & 0xFF;
////							alphaBtn.graphics.beginFill(0xaa0000);  
//							alphaBtn.graphics.beginFill(bitmapData.getPixel(x,y),alphaValue/255);  
//							alphaBtn.graphics.drawRect(x,y,1,1);
//						} 
//					}  
//				}	
//				alphaBtn.graphics.endFill();
//				alphaBtn.mouseEnabled = false;
//				this.hitArea = alphaBtn;
		...
		{
			var gp:Point = new Point(this.mouseX,this.mouseY);
			trace("ir buton down",gp.x,gp.y);
			
			if(!bitmapData.getPixel32(gp.x,gp.y))
			{
